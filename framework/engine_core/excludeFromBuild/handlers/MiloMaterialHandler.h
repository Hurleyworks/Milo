...
{
 public:
    // Factory method to create a new MiloMaterialHandler instance
    static MiloMaterialHandlerPtr create (RenderContextPtr ctx)
    {
        return std::make_shared<MiloMaterialHandler> (ctx);
    }

    // Constructor initializes the handler with texture samplers and context
    MiloMaterialHandler (RenderContextPtr ctx);
