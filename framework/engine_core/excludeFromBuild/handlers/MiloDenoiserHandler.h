...
// MiloDenoiserHandler manages OptiX AI-accelerated denoising functionality specifically for MiloEngine.
...
using MiloDenoiserHandlerPtr = std::shared_ptr<class MiloDenoiserHandler>;

class MiloDenoiserHandler
{
public:
    // Factory method following render_core pattern
    static MiloDenoiserHandlerPtr create(RenderContextPtr renderContext);

    // Constructor takes RenderContext
    explicit MiloDenoiserHandler(RenderContextPtr renderContext);
    ~MiloDenoiserHandler();
