...
ShockerModelHandler::~ShockerModelHandler()
{
    finalize();
}

// Initialize the model handler with buffer allocation for geometry instances
void ShockerModelHandler::initialize()
{
    if (isInitialized_)
    {
        LOG(WARNING) << "ShockerModelHandler already initialized";
        return;
    }
    
    LOG(INFO) << "Initializing ShockerModelHandler";
    ...
    isInitialized_ = true;
    LOG(INFO) << "ShockerModelHandler initialized successfully";
}

// Finalize and clean up all resources
void ShockerModelHandler::finalize()
{
    if (!isInitialized_)
        return;
        
    LOG(INFO) << "Finalizing ShockerModelHandler";
    ...
    try
    {
        ...
        LOG(INFO) << "ShockerModelHandler finalized successfully";
    }
    catch (const std::exception& e)
    {
        LOG(WARNING) << "Exception during ShockerModelHandler finalization: " << e.what();
    }
    
    isInitialized_ = false;
}

// Adds a single renderable node to the Shocker scene
// Creates appropriate model type based on node characteristics
void ShockerModelHandler::addCgModel(RenderableWeakRef weakNode)
{
    ...
    if (node->isInstance())
    {
        ...
        ShockerModelPtr sourceModel = getShockerModel(instancedFrom->getClientID());
        if (!sourceModel)
        {
        ...
        ShockerModelPtr shockerModel = ShockerFlyweightModel::create();
        if (!shockerModel)
        {
            LOG(WARNING) << "Failed to create ShockerModelPtr";
            return;
        }

        // Flyweight models share the geometry instance slot with their source model
        shockerModel->setGeomInstSlot(sourceModel->getGeomInstSlot());
    ...
    else if (node->isPhantom())
    {
        ...
        ShockerModelPtr shockerModel = ShockerPhantomModel::create();
    ...
    else
    {
        ...
        ShockerModelPtr shockerModel = ShockerTriangleModel::create();
        ...
        shockerModel->setGeomInstSlot(geomInstSlot);
        ...
        shockerModel->createGeometry(ctx_, node, scene);

        // Now create materials for each surface
        ShockerTriangleModel* triangleModel = dynamic_cast<ShockerTriangleModel*>(shockerModel.get());
        if (triangleModel)
        {
        ...
        if (geomInstSlot != SlotFinder::InvalidSlotIndex)
        {
            ...
            shockerModel->populateGeometryInstanceData(&geomInstDataOnHost[geomInstSlot]);
...
// Adds multiple renderable nodes to the Shocker scene efficiently
// Optimizes the process by building models first, then creating instances
void ShockerModelHandler::addCgModelList(const WeakRenderableList& weakNodeList)
{
...
void ShockerModelHandler::removeModel(ItemID itemID)
{
    // Get the model before removing it so we can release its slot
    ShockerModelPtr model = modelMgr.retrieveModel(itemID);
    if (model)
    {
        ...
        if (slot != SlotFinder::InvalidSlotIndex)
        {
            ...
            ShockerFlyweightModel* flyweight = dynamic_cast<ShockerFlyweightModel*>(model.get());
            if (!flyweight)
            {
