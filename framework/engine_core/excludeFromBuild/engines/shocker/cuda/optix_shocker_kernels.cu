    ...
    const DisneyPrincipled& bsdf, const shocker_shared::LightSample& lightSample)
{
    ...
    shocker_shared::VisibilityRayPayloadSignature::trace (
        ...
        RayType::RayType_Visibility, shocker_shared::NumRayTypes, RayType::RayType_Visibility,
        visibility);
    ...
    shocker_shared::LightSample* lightSample, float* areaPDensity)
{
    ...
    *areaPDensity = uvPDF / (2 * Pi * Pi * std::sin (theta));
    ...
    emittance = RGB (Pi * shocker_plp.f->envLightPowerCoeff);
    texCoord.x = u;
    texCoord.y = v;
    ...
    shocker_shared::LightSample* lightSample, float* areaPDensity) 
{
    *areaPDensity = 0.0f;
    ...
    if (!shocker_plp.s->enableAreaLights || shocker_plp.s->numLightInsts == 0) {
        return;
    }
    ...
    const shared::InstanceData& inst = shocker_plp.s->instanceDataBufferArray[shocker_plp.f->bufferIndex][instSlot];
    ...
    const shared::GeometryInstanceData& geomInst = shocker_plp.s->geometryInstanceDataBuffer[geomInstSlot];
    ...
    const shared::DisneyData& mat = shocker_plp.s->materialDataBuffer[geomInst.materialSlot];
    ...
    if (mat.emissive) {
        ...
        lightSample->emittance = RGB(texValue.x, texValue.y, texValue.z) * shocker_plp.s->areaLightPowerCoeff;
    } else {
    ...
    PCG32RNG& rng)
{
    ...
    if (shocker_plp.f->enableEnvLight && shocker_plp.s->envLightTexture) {
    ...
    shocker_shared::LightSample lightSample; // Sampled light information
...
CUDA_DEVICE_KERNEL void RT_RG_NAME (pathTracing)()
{
    ...
    PCG32RNG rng = shocker_plp.s->rngBuffer[launchIndex];
    ...
    if (shocker_plp.f->camera.lensSize > 0.0f)
    {
        // Use R2 sequence for pixel sampling with DOF
        uint32_t pixelIndex = launchIndex.y * shocker_plp.s->imageSize.x + launchIndex.x;
        uint32_t sampleIndex = pixelIndex + shocker_plp.f->numAccumFrames * (shocker_plp.s->imageSize.x * shocker_plp.s->imageSize.y);
        float2 r2Sample = R2Sequence(sampleIndex);
        
        Point2D pixel (
            (launchIndex.x + r2Sample.x) / shocker_plp.s->imageSize.x,
            (launchIndex.y + r2Sample.y) / shocker_plp.s->imageSize.y);

        generateCameraRay (rng, shocker_plp.f->camera, pixel, &origin, &direction);
    }
    else
    {
        ...
        float x = (launchIndex.x + jx) / shocker_plp.s->imageSize.x;
        float y = (launchIndex.y + jy) / shocker_plp.s->imageSize.y;

        // Compute vertical and horizontal view angles
        float vh = 2 * std::tan (shocker_plp.f->camera.fovY * 0.5f);
        float vw = shocker_plp.f->camera.aspect * vh;
    ...
    //    printf("ShockerEngine RG: travHandle=%llu, origin=(%.2f,%.2f,%.2f), dir=(%.2f,%.2f,%.2f)\n",
    //           shocker_plp.f->travHandle, origin.x, origin.y, origin.z, direction.x, direction.y, direction.z);
    //}
    ...
    while (true)
    {
        ...
        if (payload.terminate || payload.pathLength >= shocker_plp.f->maxPathLength)
            break;
    ...
    shocker_plp.s->rngBuffer[launchIndex] = rng;
    ...
    float curWeight = 1.0f / (1 + shocker_plp.f->numAccumFrames);

    // Clamp contribution to reduce fireflies
    RGB clampedContribution = payload.contribution;
    clampedContribution.r = fminf(clampedContribution.r, shocker_plp.s->maxRadiance);
    clampedContribution.g = fminf(clampedContribution.g, shocker_plp.s->maxRadiance);
    clampedContribution.b = fminf(clampedContribution.b, shocker_plp.s->maxRadiance);
    ...
    if (!isnan (hitPointParams.positionInWorld.x) && !isnan (hitPointParams.prevPositionInWorld.x))
    {
        ...
        Point2D prevRasterPos = shocker_plp.f->prevCamera.calcScreenPosition (hitPointParams.prevPositionInWorld) 
                               * Point2D (shocker_plp.s->imageSize.x, shocker_plp.s->imageSize.y);
    ...
    shocker_plp.s->flowAccumBuffer.write (launchIndex, make_float4 (motionVector.x, motionVector.y, 0.0f, 1.0f));
}
...
CUDA_DEVICE_KERNEL void RT_MS_NAME (miss)()
{
    ...
    if (shocker_plp.s->envLightTexture)
    {
        ...
        float4 texValue = tex2DLod<float4> (shocker_plp.s->envLightTexture, texCoord.x, texCoord.y, 0.0f);
    ...
    RGB background;
    if (shocker_plp.s->useSolidBackground || !shocker_plp.s->envLightTexture)
    {
        background = RGB (shocker_plp.s->backgroundColor.x, shocker_plp.s->backgroundColor.y, shocker_plp.s->backgroundColor.z);
    }
    else
    {
    ...
    {
        float uvPDF = shocker_plp.s->envLightImportanceMap.evaluatePDF (texCoord.x, texCoord.y);
        ...
        payload->contribution += payload->alpha * (environmentValue * shocker_plp.f->envLightPowerCoeff) * misWeight;
    }
    else
    {
        // First bounce - use raw background without power coefficient
       payload->contribution = background;
       // payload->contribution = RGB(1.0f, 0.0f, 0.0f) * shocker_plp.envLightPowerCoeff;
    }
...
CUDA_DEVICE_KERNEL void RT_CH_NAME (shading)()
{
    ...
    if (emission.r > 0.0f || emission.g > 0.0f || emission.b > 0.0f)
    {
        ...
        else if (shocker_plp.s->enableAreaLights && !payload->deltaSampled)
        {
            ...
            if (inst.isEmissive)
            {
                ...
                float instProb = shocker_plp.s->lightInstDist.evaluatePMF(optixGetInstanceId());
                ...
                float envLightProb = (shocker_plp.f->enableEnvLight && shocker_plp.s->envLightTexture) ? 0.5f : 0.0f;
...
CUDA_DEVICE_KERNEL void RT_AH_NAME (visibility)()
{
    ...
    float ior = shocker_plp.s->globalGlassIOR; // Use global glass IOR from pipeline params
    ...
    bool thinWalled = (shocker_plp.s->globalGlassType == 0);
