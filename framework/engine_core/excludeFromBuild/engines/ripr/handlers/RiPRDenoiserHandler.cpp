...
bool RiPRDenoiserHandler::initialize(uint32_t width, uint32_t height, bool useTemporalDenoiser)
{
    if (initialized_)
    {
        LOG(DBUG) << "RiPRDenoiserHandler already initialized";
        return true;
    }

    if (!renderContext_ || !renderContext_->isInitialized())
    {
        LOG(WARNING) << "RiPRDenoiserHandler: RenderContext not initialized";
        return false;
    }
    ...
    try
    {
        ...
        {
            LOG(WARNING) << "Failed to create RiPR denoiser";
            return false;
        }

        if (!setupBuffersAndTasks(width, height))
        {
            LOG(WARNING) << "Failed to setup RiPR denoiser buffers and tasks";
            denoiser_.destroy();
            return false;
        }
    ...
    {
        LOG(WARNING) << "Failed to initialize RiPR denoiser: " << ex.what();
        finalize();
        return false;
    }
}
...
void RiPRDenoiserHandler::finalize()
{
    ...
    LOG(DBUG) << "RiPRDenoiserHandler finalized";
}
...
void RiPRDenoiserHandler::resize(uint32_t width, uint32_t height)
{
    if (!initialized_)
    {
        LOG(WARNING) << "RiPRDenoiserHandler not initialized";
        return;
    }
    ...
    if (!setupBuffersAndTasks(width, height))
    {
        LOG(WARNING) << "Failed to resize RiPR denoiser buffers and tasks";
        return;
    }

    needsStateSetup_ = true;
    LOG(DBUG) << "RiPRDenoiserHandler resized to " << width << "x" << height;
}
...
void RiPRDenoiserHandler::updateDenoiserType(bool useTemporalDenoiser)
{
    ...
    if (!createDenoiser(useTemporalDenoiser))
    {
        LOG(WARNING) << "Failed to create new RiPR denoiser type";
        return;
    }
    ...
    {
        LOG(WARNING) << "Failed to setup buffers for new RiPR denoiser type";
        return;
    }

    needsStateSetup_ = true;
    LOG(INFO) << "RiPRDenoiserHandler type updated to " << (useTemporalDenoiser ? "temporal" : "HDR");
}
...
bool RiPRDenoiserHandler::createDenoiser(bool useTemporalDenoiser)
{
...
bool RiPRDenoiserHandler::setupBuffersAndTasks(uint32_t width, uint32_t height)
{
    ...
    LOG(DBUG) << "RiPR Denoiser State Buffer: " << denoiserSizes.stateSize << " bytes";
    LOG(DBUG) << "RiPR Denoiser Scratch Buffer: " << denoiserSizes.scratchSize << " bytes";
    LOG(DBUG) << "RiPR Compute Intensity Scratch Buffer: " << denoiserSizes.scratchSizeForComputeNormalizer << " bytes";
