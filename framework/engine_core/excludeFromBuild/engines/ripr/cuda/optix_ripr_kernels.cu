    ...
    ripr_shared::LightSample* lightSample, float* areaPDensity)
{
    ...
    float posPhi = phi - ripr_plp.s->envLightRotation;
    ...
    *areaPDensity = uvPDF / (2 * Pi * Pi * std::sin (theta));
    ...
    emittance = RGB (Pi * ripr_plp.s->envLightPowerCoeff);
    texCoord.x = u;
    texCoord.y = v;
    ...
    ripr_shared::LightSample* lightSample, float* areaPDensity) 
{
    *areaPDensity = 0.0f;
    ...
    const shared::InstanceData& inst = ripr_plp.s->instanceDataBufferArray[ripr_plp.s->bufferIndex][instSlot];
    ...
    PCG32RNG& rng)
{
    ...
    if (ripr_plp.s->enableEnvLight && ripr_plp.s->envLightTexture) {
...
CUDA_DEVICE_KERNEL void RT_RG_NAME (pathTracing)()
{
    ...
    if (ripr_plp.s->camera.lensSize > 0.0f)
    {
        // Use R2 sequence for pixel sampling with DOF
        uint32_t pixelIndex = launchIndex.y * ripr_plp.s->imageSize.x + launchIndex.x;
        uint32_t sampleIndex = pixelIndex + ripr_plp.s->numAccumFrames * (ripr_plp.s->imageSize.x * ripr_plp.s->imageSize.y);
        ...
        generateCameraRay (rng, ripr_plp.s->camera, pixel, &origin, &direction);
    }
    else
    {
        ...
        float vh = 2 * std::tan (ripr_plp.s->camera.fovY * 0.5f);
        float vw = ripr_plp.s->camera.aspect * vh;
    ...
    //           ripr_plp.s->travHandle, origin.x, origin.y, origin.z, direction.x, direction.y, direction.z);
    //}
    ...
    while (true)
    {
        ...
        if (payload.terminate || payload.pathLength >= ripr_plp.s->bounceLimit)
            break;
    ...
    {
        prevColorResult = RGB (getXYZ (ripr_plp.s->colorAccumBuffer.read (launchIndex)));
    ...
    float curWeight = 1.0f / (1 + ripr_plp.s->numAccumFrames);
...
#endif
    ripr_plp.s->colorAccumBuffer.write (launchIndex, make_float4 (colorResult.toNative(), 1.0f));
    ...
    if (!isnan (hitPointParams.positionInWorld.x) && !isnan (hitPointParams.prevPositionInWorld.x))
    {
        ...
        Point2D prevRasterPos = ripr_plp.s->prevCamera.calcScreenPosition (hitPointParams.prevPositionInWorld) 
...
CUDA_DEVICE_KERNEL void RT_MS_NAME (miss)()
{
    ...
    if (ripr_plp.s->envLightTexture)
    {
        ...
        float phi = posPhi + ripr_plp.s->envLightRotation;
    ...
    if (payload->pathLength > 1 && !payload->deltaSampled)
    {
        ...
        payload->contribution += payload->alpha * (environmentValue * ripr_plp.s->envLightPowerCoeff) * misWeight;
    }
    else
    {
    ...
    if (emission.r > 0.0f || emission.g > 0.0f || emission.b > 0.0f)
    {
        ...
        else if (ripr_plp.s->enableAreaLights && !payload->deltaSampled)
        {
            ...
            if (inst.isEmissive)
            {
                ...
                float envLightProb = (ripr_plp.s->enableEnvLight && ripr_plp.s->envLightTexture) ? 0.5f : 0.0f;
