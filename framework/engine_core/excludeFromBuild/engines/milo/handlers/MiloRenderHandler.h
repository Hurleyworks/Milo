...
// MiloRenderHandler - Render handler for MiloEngine
...
using MiloRenderHandlerPtr = std::shared_ptr<class MiloRenderHandler>;

class MiloRenderHandler
{
public:
    // Factory method
    static MiloRenderHandlerPtr create(RenderContextPtr ctx) 
    { 
        return std::make_shared<MiloRenderHandler>(ctx); 
    }
    
    MiloRenderHandler(RenderContextPtr ctx);
    ~MiloRenderHandler();
    ...
    bool denoise(CUstream stream, bool isNewSequence, class MiloDenoiserHandler* denoiserHandler, struct GPUTimerManager::GPUTimer* timer = nullptr);
...
private:
    ...
    // Accumulation buffers (2D arrays for Milo path tracing output)
