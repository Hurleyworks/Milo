...
MiloMaterialHandler::MiloMaterialHandler (RenderContextPtr ctx) :
    ctx (ctx)
{
    LOG (DBUG) << _FN_;
...
MiloMaterialHandler::~MiloMaterialHandler()
{
    LOG (DBUG) << _FN_;
    finalize();
}
...
void MiloMaterialHandler::initialize()
{
    if (isInitialized_)
    {
        LOG(WARNING) << "MiloMaterialHandler already initialized";
        return;
    }
    
    LOG(INFO) << "Initializing MiloMaterialHandler";
    ...
    isInitialized_ = true;
    LOG(INFO) << "MiloMaterialHandler initialized successfully";
}

// Safely releases all texture and material resources
void MiloMaterialHandler::finalize()
{
    if (!isInitialized_)
        return;
        
    LOG(INFO) << "Finalizing MiloMaterialHandler";
    ...
    try
    {
        ...
        LOG (INFO) << "MiloMaterialHandler finalized";
    }
...
shared::TexDimInfo MiloMaterialHandler::calcDimInfo (
...
optixu::Material MiloMaterialHandler::createOptixMaterial()
{
    optixu::Material mat = ctx->getOptiXContext().createMaterial();

    // NOTE: Hit groups will be set by MiloEngine after pipelines are created
    // This differs from production code where PipelineHandler is accessible via Handlers
    
    return mat;
}

optixu::Material MiloMaterialHandler::createDisneyMaterial (const CgMaterial& material,
                                                        const std::filesystem::path& materialFolder, CgModelPtr model)
{
    ...
    if (!isInitialized_)
    {
        LOG(WARNING) << "MiloMaterialHandler not initialized - falling back to legacy mode";
...
void MiloMaterialHandler::processTextureInfo (
...
void MiloMaterialHandler::updateMaterial (
