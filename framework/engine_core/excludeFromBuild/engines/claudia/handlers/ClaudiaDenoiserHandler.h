...
// ClaudiaDenoiserHandler manages OptiX AI-accelerated denoising functionality specifically for ClaudiaEngine.
// This is a dedicated denoiser handler for ClaudiaEngine to avoid conflicts with other engines.
...
using ClaudiaDenoiserHandlerPtr = std::shared_ptr<class ClaudiaDenoiserHandler>;

class ClaudiaDenoiserHandler
{
public:
    // Factory method following render_core pattern
    static ClaudiaDenoiserHandlerPtr create(RenderContextPtr renderContext);

    // Constructor takes RenderContext
    explicit ClaudiaDenoiserHandler(RenderContextPtr renderContext);
    ~ClaudiaDenoiserHandler();
