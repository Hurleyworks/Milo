...
// Forward declaration
using ClaudiaSceneHandlerPtr = std::shared_ptr<class ClaudiaSceneHandler>;

// Forward declarations
class ClaudiaModelHandler;
using ClaudiaModelHandlerPtr = std::shared_ptr<ClaudiaModelHandler>;

class ClaudiaSceneHandler
{
 public:
    // Factory function for creating ClaudiaSceneHandler objects
    static ClaudiaSceneHandlerPtr create (RenderContextPtr ctx) { return std::make_shared<ClaudiaSceneHandler> (ctx); }
 ...
 public:
    ...
    ClaudiaSceneHandler (RenderContextPtr ctx);

    // Destructor handles cleanup of scene resources
    ~ClaudiaSceneHandler();

    // Set the model handler (must be called before using model operations)
    void setModelHandler(ClaudiaModelHandlerPtr modelHandler) { modelHandler_ = modelHandler; }
 ...
 private:
    ...
    // Model handler for managing Claudia models
    ClaudiaModelHandlerPtr modelHandler_;
