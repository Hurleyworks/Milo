...
using ClaudiaMaterialHandlerPtr = std::shared_ptr<class ClaudiaMaterialHandler>;
using sabi::CgModelPtr;
using sabi::CgTextureInfo;

class ClaudiaMaterialHandler
{
 public:
    // Factory method to create a new ClaudiaMaterialHandler instance
    static ClaudiaMaterialHandlerPtr create (RenderContextPtr ctx)
    {
        return std::make_shared<ClaudiaMaterialHandler> (ctx);
    }

    // Constructor initializes the handler with texture samplers and context
    ClaudiaMaterialHandler (RenderContextPtr ctx);

    // Destructor cleans up materials and texture resources
    ~ClaudiaMaterialHandler();
