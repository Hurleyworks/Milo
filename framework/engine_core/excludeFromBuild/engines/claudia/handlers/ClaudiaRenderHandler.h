...
// ClaudiaRenderHandler - Render handler for ClaudiaEngine
// Manages accumulation buffers that Claudia path tracer writes to
// Based on ShockerRenderHandler but adapted for Claudia's specific needs

#include "../../../common/common_host.h"

class RenderContext;
using RenderContextPtr = std::shared_ptr<RenderContext>;
using ClaudiaRenderHandlerPtr = std::shared_ptr<class ClaudiaRenderHandler>;

class ClaudiaRenderHandler
{
public:
    // Factory method
    static ClaudiaRenderHandlerPtr create(RenderContextPtr ctx) 
    { 
        return std::make_shared<ClaudiaRenderHandler>(ctx); 
    }
    
    ClaudiaRenderHandler(RenderContextPtr ctx);
    ~ClaudiaRenderHandler();
    ...
    bool denoise(CUstream stream, bool isNewSequence, class ClaudiaDenoiserHandler* denoiserHandler, struct GPUTimerManager::GPUTimer* timer = nullptr);
...
private:
    ...
    // Accumulation buffers (2D arrays for Claudia path tracing output)
