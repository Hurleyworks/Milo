...
// https://github.com/ripr-0x15/OptiX_Utility/blob/master/LICENSE.md
...
namespace ripr_shared
{
        ...
        ripr_shared::LightSample* lightSample, float* areaPDensity)
    {
        ...
        else
        {
            ...
            lightProb *= instProb;
            const InstanceData& inst = ripr_plp.s->instanceDataBufferArray[ripr_plp.f->bufferIndex][instSlot];
            if (instProb == 0.0f)
            {
                *areaPDensity = 0.0f;
                return;
        ...
        float* hypAreaPDensity)
    {
        ...
        *texCoord0DirInWorld = normalize (transformVectorFromObjectToWorldSpace (texCoord0DirInObj));
        ...
        if constexpr (computeHypotheticalAreaPDensity)
        {
            ...
            lightProb *= (pow2 (inst.uniformScale) * instImportance) / ripr_plp.s->lightInstDist.integral();
