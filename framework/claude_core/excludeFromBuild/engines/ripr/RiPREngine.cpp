...
{
    LOG (INFO) << "RiPREngine::onEnvironmentChanged()";
...
{
    LOG (INFO) << "RiPREngine::setupPipelines() - Using PipelineHandler";
    ...
    pathTraceData.numRayTypes = ripr_shared::PathTracingRayType::NumTypes;
    pathTraceData.searchRayIndex = ripr_shared::PathTracingRayType::Closest;
    pathTraceData.visibilityRayIndex = ripr_shared::PathTracingRayType::Visibility;

    // Configure pipeline options
    pathTraceData.config.maxTraceDepth = 8;
    pathTraceData.config.numPayloadDwords = std::max ({ripr_shared::PathTraceRayPayloadSignature::numDwords,
                                                       ripr_shared::VisibilityRayPayloadSignature::numDwords});
    pathTraceData.config.numAttributeDwords = optixu::calcSumDwords<float2>();
    pathTraceData.config.launchParamsName = "ripr_plp";
    pathTraceData.config.launchParamsSize = sizeof (ripr_shared::PipelineLaunchParameters);
    ...
    pipelineHandler->setupPipeline (pathTraceData, std::string ("optix_ripr_kernels"));
    ...
    gbufferData.numRayTypes = ripr_shared::GBufferRayType::NumTypes;
    gbufferData.searchRayIndex = ripr_shared::GBufferRayType::Primary;

    // Configure GBuffer pipeline options
    gbufferData.config.maxTraceDepth = 2;
    gbufferData.config.numPayloadDwords = ripr_shared::PrimaryRayPayloadSignature::numDwords;
    gbufferData.config.numAttributeDwords = optixu::calcSumDwords<float2>();
    gbufferData.config.launchParamsName = "ripr_plp";
    gbufferData.config.launchParamsSize = sizeof (ripr_shared::PipelineLaunchParameters);
    ...
    pipelineHandler->setupPipeline (gbufferData, std::string ("optix_ripr_gbuffer"));
...
void RiPREngine::updateMaterialHitGroups (RiPRModelPtr model)
{
    ...
    // Define hit group configuration for RiPR pipelines
    ...
    hitGroupConfig[EntryPointType::PathTrace] = {
        {ripr_shared::PathTracingRayType::Closest, RT_CH_NAME_STR ("pathTraceBaseline")}, // Ray type 0: shading
        {ripr_shared::PathTracingRayType::Visibility, RT_AH_NAME_STR ("visibility")}      // Ray type 1: shadow rays
    };

    // GBuffer pipeline configuration
    hitGroupConfig[EntryPointType::GBuffer] = {
        {ripr_shared::GBufferRayType::Primary, RT_CH_NAME_STR ("setupGBuffers")} // Ray type 0: primary rays
    };
...
{
    LOG (INFO) << "RiPREngine::allocateLaunchParameters()";
...
void RiPREngine::outputGBufferDebugInfo (CUstream stream)
{
    ...
    // Read pick info - exactly like RiPR code
