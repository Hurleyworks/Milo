...
RiPRAreaLightHandler::RiPRAreaLightHandler(RenderContextPtr ctx)
    : ctx_(ctx)
{
    LOG(DBUG) << _FN_;
}

RiPRAreaLightHandler::~RiPRAreaLightHandler()
{
    finalize();
}

void RiPRAreaLightHandler::initialize()
{
    if (isInitialized_)
    {
        LOG(WARNING) << "RiPRAreaLightHandler already initialized";
        return;
    }
    
    LOG(INFO) << "Initializing RiPRAreaLightHandler";
    ...
    isInitialized_ = true;
    LOG(INFO) << "RiPRAreaLightHandler initialized successfully";
}

void RiPRAreaLightHandler::finalize()
{
    if (!isInitialized_)
        return;
        
    LOG(INFO) << "Finalizing RiPRAreaLightHandler";
    ...
    try
    {
        ...
        LOG(INFO) << "RiPRAreaLightHandler finalized successfully";
    }
    catch (const std::exception& e)
    {
        LOG(WARNING) << "Exception during RiPRAreaLightHandler finalization: " << e.what();
    }
    
    isInitialized_ = false;
}

void RiPRAreaLightHandler::prepareSceneLightDistribution(uint32_t maxInstances)
{
...
void RiPRAreaLightHandler::updateSceneLightDistribution(CUstream stream, uint32_t bufferIndex)
{
    if (!sceneHandler_)
    {
...
void RiPRAreaLightHandler::setupSceneLightSampling(CUstream stream, CUdeviceptr lightInstDistAddr, uint32_t bufferIndex)
{
...
void RiPRAreaLightHandler::prepareInstanceLightDistribution(RiPRModel* model)
{
    if (!model)
        return;
...
void RiPRAreaLightHandler::updateInstanceLightDistribution(CUstream stream, RiPRModel* model, uint32_t instSlot)
{
    if (!model)
        return;
...
void RiPRAreaLightHandler::markInstanceDirty(RiPRModel* model)
{
    if (model)
    {
        dirtyInstances_.insert(model);
...
void RiPRAreaLightHandler::prepareGeometryLightDistribution(RiPRTriangleModel* triModel, uint32_t numTriangles)
{
    ...
    // initialized in RiPRModelHandler::computeLightProbabilities
...
void RiPRAreaLightHandler::updateGeometryLightDistribution(CUstream stream, RiPRTriangleModel* triModel, uint32_t geomInstSlot, uint32_t materialSlot)
{
    if (!triModel)
        return;
...
void RiPRAreaLightHandler::markGeometryDirty(RiPRTriangleModel* triModel)
{
    if (triModel)
    {
        dirtyGeometries_.insert(triModel);
...
void RiPRAreaLightHandler::rebuildAllDistributions(CUstream stream)
{
...
void RiPRAreaLightHandler::updateDirtyDistributions(CUstream stream, uint32_t bufferIndex)
{
...
float RiPRAreaLightHandler::getTotalLightPower() const
{
    // TODO: Calculate total light power from all distributions
    return 1.0f;
}

void RiPRAreaLightHandler::resizeScratchMemory(size_t requiredSize)
{
...
void RiPRAreaLightHandler::computeTriangleProbabilities(
    CUstream stream, 
...
void RiPRAreaLightHandler::computeGeomInstProbabilities(
    CUstream stream,
    uint32_t instSlot,
...
void RiPRAreaLightHandler::computeInstProbabilities(CUstream stream, uint32_t numInsts)
{
    if (!sceneHandler_)
        return;
...
void RiPRAreaLightHandler::finalizeLightDistribution(
    CUstream stream,
    LightDistribution& dist,
...
void RiPRAreaLightHandler::finalizeEmitterDistribution(
    CUstream stream,
    RiPRModelPtr model, 
    uint32_t geomInstSlot)
{
    ...
    auto triangleModel = std::dynamic_pointer_cast<RiPRTriangleModel>(model);
    if (!triangleModel)
    {
...
uint2 RiPRAreaLightHandler::computeProbabilityTextureDimensions(uint32_t numElements) const
{
...
void RiPRAreaLightHandler::initializeLightKernels()
{
    if (!ctx_)
    {
...
void RiPRAreaLightHandler::cleanupLightKernels()
{
    if (computeProbTex_.cudaModule)
    {
        try
