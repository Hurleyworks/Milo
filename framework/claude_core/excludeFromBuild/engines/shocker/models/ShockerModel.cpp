...
void ShockerTriangleModel::createGeometry(RenderContextPtr ctx, RenderableNode& node, optixu::Scene scene)
{
    ...
    // Materials will be created separately by ShockerMaterialHandler in ShockerModelHandler::addCgModel
...
void ShockerTriangleModel::createGAS (RenderContextPtr ctx, optixu::Scene scene, uint32_t numRayTypes, cudau::Buffer& scratchMem)
{
    ...
    gasData.gas.setMaterialSetCount(ShockerConstants::MATERIAL_SETS);
    for (int i = 0; i < ShockerConstants::MATERIAL_SETS; ++i)
...
void ShockerTriangleModel::extractVertexPositions(MatrixXf& V)
{
...
void ShockerTriangleModel::extractTriangleIndices(MatrixXu& F)
{
