...
void ShockerRenderHandler::clearAccumBuffers(CUstream stream)
{
    ...
    // For now, we'll rely on the Shocker kernels to handle initialization
    // The clearAccumBuffers kernel is available in optix_shocker_copybuffers if needed
    LOG(DBUG) << "Accumulation buffer clearing requested - Shocker kernels will handle initialization";
}
