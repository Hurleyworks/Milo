...
void ShockerSceneHandler::finalize()
{
    try
    {
    ...
    {
        LOG (WARNING) << "Error during ShockerSceneHandler cleanup: " << e.what();
    }
}
...
void ShockerSceneHandler::init()
{
    ...
    try
    {
        ...
        ptxData = ctx->getPTXManager()->getPTXData ("optix_shocker_deform_kernels", useEmbedded);
...
#if 0
void ShockerSceneHandler::initializeSceneDependentSBT (EntryPointType type)
{
    ...
    // TODO: Implement pipeline handling in ShockerEngine
    if (!pipeline)
    ...
    // TODO: Implement SBT generation in ShockerEngine
    ...
    // TODO: Implement SBT initialization in ShockerEngine
...
void ShockerSceneHandler::resizeSceneDependentSBT()
{
    if (!scene_)
    {
...
void ShockerSceneHandler::undeformNode (RenderableNode node)
{
    ...
    auto triangleModel = std::dynamic_pointer_cast<ShockerTriangleModel> (optiXModel);
...
void ShockerSceneHandler::updateDeformedNode (RenderableNode node)
{
    ShockerModelPtr optiXModel = modelHandler_->getShockerModel (node->getClientID());
    if (!optiXModel)
    {
...
void ShockerSceneHandler::removeNodesByIDs (const std::vector<BodyID>& bodyIDs)
{
...
void ShockerSceneHandler::createInstance (RenderableWeakRef& weakNode)
{
    // debug_assert (false);
...
void ShockerSceneHandler::createGeometryInstance (RenderableWeakRef& weakNode)
{
    if (weakNode.expired()) return;
    ...
    ShockerModelPtr instancedFromModel = modelHandler_->getShockerModel (instancedFrom->getClientID());
...
void ShockerSceneHandler::createPhysicsPhantom (RenderableWeakRef& weakNode)
{
    ...
    try
    {
        ...
        ShockerModelPtr phantomFromModel = modelHandler_->getShockerModel (phantomFrom->getID());
...
void ShockerSceneHandler::createInstanceList (const WeakRenderableList& weakNodeList)
{
    ...
    for (const auto& weakNode : weakNodeList)
    {
        ...
        if (node->isInstance())
        {
            ...
            ShockerModelPtr instancedFromModel = modelHandler_->getShockerModel (instancedFrom->getID());
        ...
        else
        {
            ...
            if (!optiXModel)
            {
                LOG (CRITICAL) << "Failed to find ShockerModelPtr";
                continue;
            }
...
bool ShockerSceneHandler::updateMotion()
{
...
void ShockerSceneHandler::toggleSelectionMaterial (RenderableNode& node)
{
...
void ShockerSceneHandler::deselectAll()
{
    for (auto& it : nodeMap)
    {
        if (it.second.expired()) continue;
