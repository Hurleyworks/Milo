...
CUDA_DEVICE_KERNEL void RT_RG_NAME (setupGBuffers)()
{
    ...
    PrimaryRayPayloadSignature::trace (
        claudia_plp.f->travHandle, origin.toNative(), direction.toNative(),
    ...
    const Point2D prevRasterPos =
        claudia_plp.f->prevCamera.calcScreenPosition (hitPointParams.prevPositionInWorld) * Point2D (claudia_plp.s->imageSize.x, claudia_plp.s->imageSize.y);
    Vector2D motionVector = curRasterPos - prevRasterPos;
    // FIXME
    //  if (claudia_plp.f->resetFlowBuffer || isnan (hitPointParams.prevPositionInWorld.x))
    ...
    claudia_plp.s->GBuffer0[bufIdx].write (launchIndex, gb0Elems);
    claudia_plp.s->GBuffer1[bufIdx].write (launchIndex, gb1Elems);

    if (launchIndex.x == claudia_plp.f->mousePosition.x &&
        launchIndex.y == claudia_plp.f->mousePosition.y)
    {
        ...
        *claudia_plp.s->pickInfos[bufIdx] = pickInfo;
    }
...
CUDA_DEVICE_KERNEL void RT_CH_NAME (setupGBuffers)()
{
    ...
    const shared::InstanceData& inst = claudia_plp.s->instanceDataBufferArray[bufIdx][instanceId];
...
CUDA_DEVICE_KERNEL void RT_MS_NAME (setupGBuffers)()
{
    ...
    const float phi = posPhi + claudia_plp.f->envLightRotation;
        ...
        launchIndex.y == claudia_plp.f->mousePosition.y)
    {
        ...
        if (claudia_plp.s->envLightTexture)
        {
            ...
            emittance = RGB (getXYZ (texValue));
            emittance *= pi_v<float> * claudia_plp.f->envLightPowerCoeff;
        }
        pickInfo->emittance = emittance;
    }
}