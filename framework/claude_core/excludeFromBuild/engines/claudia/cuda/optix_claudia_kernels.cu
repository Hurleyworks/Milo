...
// https://github.com/claudia-0x15/OptiX_Utility/blob/master/LICENSE.md
// and from Claudia GfxExp
// https://github.com/claudia-0x15/GfxEx
...
CUDA_DEVICE_FUNCTION CUDA_INLINE RGB performDirectLighting (
    ...
    const DisneyPrincipled& bsdf, const claudia_shared::LightSample& lightSample)
{
    ...
    if constexpr (withVisibility)
    {
        ...
        claudia_shared::VisibilityRayPayloadSignature::trace (
            claudia_plp.f->travHandle,
            shadingPoint.toNative(), shadowRayDir.toNative(), 0.0f, dist * 0.9999f, 0.0f,
            0xFF, OPTIX_RAY_FLAG_NONE,
            RayType::Visibility, claudia_shared::maxNumRayTypes, RayType::Visibility,
            visibility);
    }
...
CUDA_DEVICE_FUNCTION CUDA_INLINE bool evaluateVisibility (
    const Point3D& shadingPoint, const claudia_shared::LightSample& lightSample)
{
    using namespace shared;
    Vector3D shadowRayDir = lightSample.atInfinity ? Vector3D (lightSample.position) : (lightSample.position - shadingPoint);
    const float dist2 = shadowRayDir.sqLength();
    ...
    const DisneyPrincipled& bsdf, PCG32RNG& rng)
{
    ...
    if constexpr (useExplicitLightSampling)
    {
        ...
        if (claudia_plp.s->envLightTexture && claudia_plp.f->enableEnvLight)
        {
...
CUDA_DEVICE_FUNCTION CUDA_INLINE void pathTrace_rayGen_generic()
{
    ...
    const uint32_t bufIdx = claudia_plp.f->bufferIndex;

    // Read GBuffer data
    const GBuffer0Elements gb0Elems = claudia_plp.s->GBuffer0[bufIdx].read (launchIndex);
    const GBuffer1Elements gb1Elems = claudia_plp.s->GBuffer1[bufIdx].read (launchIndex);
    ...
    const PerspectiveCamera& camera = claudia_plp.f->camera;

    const bool useEnvLight = claudia_plp.s->envLightTexture && claudia_plp.f->enableEnvLight;
    RGB contribution (0.0f, 0.0f, 0.0f);
    if (instSlot != 0xFFFFFFFF)
    {
        const uint32_t geomInstSlot = gb0Elems.geomInstSlot;
        const InstanceData& inst = claudia_plp.s->instanceDataBufferArray[bufIdx][instSlot];
        const GeometryInstanceData& geomInst = claudia_plp.s->geometryInstanceDataBuffer[geomInstSlot];
        ...
        float dirPDensity;
        {
            ...
            const shared::DisneyData& mat = claudia_plp.s->materialDataBuffer[materialSlot];
            ...
            if (claudia_plp.f->enableBumpMapping)
            {
        ...
        while (true)
        {
            ...
            if (rwPayload.pathLength >= claudia_plp.f->maxPathLength)
            ...
            PathTraceRayPayloadSignature::trace (
                claudia_plp.f->travHandle, rayOrg.toNative(), rayDir.toNative(),
        ...
        claudia_plp.s->rngBuffer.write (launchIndex, rwPayload.rng);
    }
    else
    {
        ...
        if (useEnvLight)
        {
            const float4 texValue = tex2DLod<float4> (claudia_plp.s->envLightTexture, bcB, bcC, 0.0f);
            const RGB luminance = claudia_plp.f->envLightPowerCoeff * RGB (getXYZ (texValue));
            contribution = luminance;
        }
    }

    RGB prevColorResult (0.0f, 0.0f, 0.0f);
    if (claudia_plp.f->numAccumFrames > 0)
        prevColorResult = RGB (getXYZ (claudia_plp.s->beautyAccumBuffer.read (launchIndex)));
    const float curWeight = 1.0f / (1 + claudia_plp.f->numAccumFrames);
    const RGB colorResult = (1 - curWeight) * prevColorResult + curWeight * contribution;

    claudia_plp.s->beautyAccumBuffer.write (launchIndex, make_float4 (colorResult.toNative(), 1.0f));
}
...
CUDA_DEVICE_FUNCTION CUDA_INLINE void pathTrace_closestHit_generic()
{
    ...
    const InstanceData& inst = claudia_plp.s->instanceDataBufferArray[bufIdx][instanceId];
    const GeometryInstanceData& geomInst = claudia_plp.s->geometryInstanceDataBuffer[sbtr.geomInstSlot];
...
CUDA_DEVICE_KERNEL void RT_MS_NAME (pathTraceBaseline)()
{
    if constexpr (useImplicitLightSampling)
    {
        ...
        float phi = posPhi + claudia_plp.f->envLightRotation;
        phi = phi - floorf (phi / (2 * pi_v<float>)) * 2 * pi_v<float>;
        const Point2D texCoord (phi / (2 * pi_v<float>), theta / pi_v<float>);

        // Implicit Light Sampling
        const float4 texValue = tex2DLod<float4> (claudia_plp.s->envLightTexture, texCoord.x, texCoord.y, 0.0f);
        RGB luminance = claudia_plp.f->envLightPowerCoeff * RGB (getXYZ (texValue));
        float misWeight = 1.0f;
        if constexpr (useMultipleImportanceSampling)
        {
            ...
            const float uvPDF = claudia_plp.s->envLightImportanceMap.evaluatePDF (u, v);
            const float hypAreaPDensity = uvPDF / (2 * pi_v<float> * pi_v<float> * std::sin (theta));
            const float lightPDensity =
                (claudia_plp.s->lightInstDist.integral() > 0.0f ? probToSampleEnvLight : 1.0f) *
                hypAreaPDensity;
            const float bsdfPDensity = rwPayload->prevDirPDensity;
            misWeight = pow2 (bsdfPDensity) / (pow2 (bsdfPDensity) + pow2 (lightPDensity));
        }
