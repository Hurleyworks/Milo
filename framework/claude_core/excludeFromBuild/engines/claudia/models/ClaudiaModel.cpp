...
// Implementation for ClaudiaTriangleModel
void ClaudiaTriangleModel::createGeometry(RenderContextPtr ctx, RenderableNode& node, optixu::Scene scene)
{
...
void ClaudiaTriangleModel::createGAS (RenderContextPtr ctx, optixu::Scene scene, uint32_t numRayTypes, cudau::Buffer& scratchMem)
{
    if (!scene)
    ...
    gasData.gas.setMaterialSetCount(ClaudiaConstants::MATERIAL_SETS);
    for (int i = 0; i < ClaudiaConstants::MATERIAL_SETS; ++i)
...
void ClaudiaTriangleModel::extractVertexPositions(MatrixXf& V)
{
...
void ClaudiaTriangleModel::extractTriangleIndices(MatrixXu& F)
{
