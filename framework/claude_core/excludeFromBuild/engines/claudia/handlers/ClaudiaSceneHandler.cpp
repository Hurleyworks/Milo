...
ClaudiaSceneHandler::ClaudiaSceneHandler (RenderContextPtr ctx) :
    ctx (ctx)
{
    LOG (DBUG) << _FN_;
    
    // Initialize the generic scene handler and configure it for Claudia's needs
    instanceHandler_ = ctx->getHandlers().intanceHandler;
    if (instanceHandler_)
    {
        // Configure for fast build with update capability (Claudia needs frequent updates)
        instanceHandler_->setConfiguration(
            optixu::ASTradeoff::PreferFastBuild,
            true,   // allowUpdate - Claudia needs to update transforms
            false   // allowCompaction - not needed for Claudia
        );
    }
}

// Cleans up displacement and CUDA module resources
ClaudiaSceneHandler::~ClaudiaSceneHandler()
{
    finalize();
}

void ClaudiaSceneHandler::finalize()
{
    try
    {
    ...
    {
        LOG (WARNING) << "Error during ClaudiaSceneHandler cleanup: " << e.what();
    }
}

void ClaudiaSceneHandler::init()
{
    LOG (DBUG) << _FN_;
    ...
    try
    {
        ...
        ptxData = ctx->getPTXManager()->getPTXData ("optix_claudia_deform_kernels", useEmbedded);
...
#if 0
void ClaudiaSceneHandler::initializeSceneDependentSBT (EntryPointType type)
{
    ...
    // TODO: Implement pipeline handling in ClaudiaEngine
    if (!pipeline)
    ...
    // TODO: Implement SBT generation in ClaudiaEngine
    ...
    // TODO: Implement SBT initialization in ClaudiaEngine
...
void ClaudiaSceneHandler::resizeSceneDependentSBT()
{
    if (!scene_)
    {
...
void ClaudiaSceneHandler::undeformNode (RenderableNode node)
{
    ClaudiaModelPtr optiXModel = modelHandler_->getClaudiaModel (node->getClientID());
    if (!optiXModel)
    {
    ...
    auto triangleModel = std::dynamic_pointer_cast<ClaudiaTriangleModel> (optiXModel);
...
void ClaudiaSceneHandler::updateDeformedNode (RenderableNode node)
{
    ClaudiaModelPtr optiXModel = modelHandler_->getClaudiaModel (node->getClientID());
    if (!optiXModel)
    {
...
void ClaudiaSceneHandler::removeNodesByIDs (const std::vector<BodyID>& bodyIDs)
{
...
void ClaudiaSceneHandler::createInstance (RenderableWeakRef& weakNode)
{
    // debug_assert (false);
    ...
    // ClaudiaModelPtr optiXModel = modelHandler_->getClaudiaModel (node->getID());
    ClaudiaModelPtr optiXModel = modelHandler_->getClaudiaModel (node->getClientID());
...
void ClaudiaSceneHandler::createGeometryInstance (RenderableWeakRef& weakNode)
{
    if (weakNode.expired()) return;
    ...
    ClaudiaModelPtr instancedFromModel = modelHandler_->getClaudiaModel (instancedFrom->getClientID());
    ...
    // Set the flag to indicate this node is stored in ClaudiaSceneHandler
    node->getState().state |= sabi::PRenderableState::StoredInSceneHandler;
...
void ClaudiaSceneHandler::createPhysicsPhantom (RenderableWeakRef& weakNode)
{
    if (weakNode.expired()) return;
    try
    {
        ...
        ClaudiaModelPtr phantomFromModel = modelHandler_->getClaudiaModel (phantomFrom->getID());
...
void ClaudiaSceneHandler::createInstanceList (const WeakRenderableList& weakNodeList)
{
    if (!instanceHandler_ || instanceHandler_->getTraversableHandle() == 0)
        init();
    ...
    for (const auto& weakNode : weakNodeList)
    {
        ...
        if (node->isInstance())
        {
            ...
            ClaudiaModelPtr instancedFromModel = modelHandler_->getClaudiaModel (instancedFrom->getID());
        ...
        else
        {
            ...
            if (!optiXModel)
            {
                LOG (CRITICAL) << "Failed to find ClaudiaModelPtr";
                continue;
            }
...
bool ClaudiaSceneHandler::updateMotion()
{
...
void ClaudiaSceneHandler::toggleSelectionMaterial (RenderableNode& node)
{
...
void ClaudiaSceneHandler::selectAll()
{
    for (auto& it : nodeMap)
    {
...
void ClaudiaSceneHandler::deselectAll()
{
    for (auto& it : nodeMap)
    {
        if (it.second.expired()) continue;
...
void ClaudiaSceneHandler::rebuild()
{
...
void ClaudiaSceneHandler::updateEmissiveInstances()
{
