...
{
    LOG (INFO) << "ClaudiaEngine::setupPipelines() - Using PipelineHandler";
    ...
    pathTraceData.numRayTypes = claudia_shared::PathTracingRayType::NumTypes;
    pathTraceData.searchRayIndex = claudia_shared::PathTracingRayType::Closest;
    pathTraceData.visibilityRayIndex = claudia_shared::PathTracingRayType::Visibility;

    // Configure pipeline options
    pathTraceData.config.maxTraceDepth = 8;
    pathTraceData.config.numPayloadDwords = std::max ({claudia_shared::PathTraceRayPayloadSignature::numDwords,
    ...
    pathTraceData.config.launchParamsName = "claudia_plp";
    pathTraceData.config.launchParamsSize = sizeof (claudia_shared::PipelineLaunchParameters);
    ...
    pipelineHandler->setupPipeline (pathTraceData, std::string ("optix_claudia_kernels"));
    ...
    gbufferData.numRayTypes = claudia_shared::GBufferRayType::NumTypes;
    gbufferData.searchRayIndex = claudia_shared::GBufferRayType::Primary;

    // Configure GBuffer pipeline options
    gbufferData.config.maxTraceDepth = 2;
    gbufferData.config.numPayloadDwords = claudia_shared::PrimaryRayPayloadSignature::numDwords;
    gbufferData.config.numAttributeDwords = optixu::calcSumDwords<float2>();
    gbufferData.config.launchParamsName = "claudia_plp";
    gbufferData.config.launchParamsSize = sizeof (claudia_shared::PipelineLaunchParameters);
    ...
    pipelineHandler->setupPipeline (gbufferData, std::string ("optix_claudia_gbuffer"));
...
void ClaudiaEngine::updateMaterialHitGroups (ClaudiaModelPtr model)
{
    ...
    // Define hit group configuration for Claudia pipelines
    ...
    hitGroupConfig[EntryPointType::PathTrace] = {
        {claudia_shared::PathTracingRayType::Closest, RT_CH_NAME_STR ("pathTraceBaseline")}, // Ray type 0: shading
        {claudia_shared::PathTracingRayType::Visibility, RT_AH_NAME_STR ("visibility")}      // Ray type 1: shadow rays
    };

    // GBuffer pipeline configuration
    hitGroupConfig[EntryPointType::GBuffer] = {
        {claudia_shared::GBufferRayType::Primary, RT_CH_NAME_STR ("setupGBuffers")} // Ray type 0: primary rays
    };
...
void ClaudiaEngine::updateLaunchParameters (const mace::InputEvent& input)
{
    ...
    if (screenBufferHandler && screenBufferHandler->isInitialized())
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            static_plp_.pickInfos[i] = reinterpret_cast<claudia_shared::PickInfo*> (
...
void ClaudiaEngine::outputGBufferDebugInfo (CUstream stream)
{
    ...
    // Read pick info - exactly like Claudia code
