...
namespace claudia_shared
{
    ...
    CUDA_DEVICE_FUNCTION CUDA_INLINE void sampleLight (
        ...
        claudia_shared::LightSample* lightSample, float* areaPDensity)
    {
        using namespace shared;
        ...
        if (sampleEnvLight)
        {
            ...
            float posPhi = phi - claudia_plp.f->envLightRotation;
            ...
            *areaPDensity = uvPDF / (2 * pi_v<float> * pi_v<float> * sinTheta);
            ...
            emittance = RGB (pi_v<float> * claudia_plp.f->envLightPowerCoeff);
            texCoord.x = u;
            texCoord.y = v;
        }
        else
        {
            ...
            lightProb *= instProb;
            const InstanceData& inst = claudia_plp.s->instanceDataBufferArray[claudia_plp.f->bufferIndex][instSlot];
            if (instProb == 0.0f)
            {
                *areaPDensity = 0.0f;
                return;
            ...
            const DisneyData& mat = claudia_plp.s->materialDataBuffer[geomInst.materialSlot];
        ...
        float* hypAreaPDensity)
    {
        ...
        *texCoord0DirInWorld = normalize (transformVectorFromObjectToWorldSpace (texCoord0DirInObj));
        ...
        if constexpr (computeHypotheticalAreaPDensity)
        {
            ...
            lightProb *= (pow2 (inst.uniformScale) * instImportance) / claudia_plp.s->lightInstDist.integral();
