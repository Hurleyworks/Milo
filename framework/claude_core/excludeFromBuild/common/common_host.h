...
class ProbabilityTexture
{
 ...
 public:
    ProbabilityTexture() :
        m_cuTexObj (0), m_isInitialized (false) {}
...
struct Scene
{
        ...
        uint32_t _numRayTypes)
    {
        ...
        constexpr int32_t maxScanSize = std::max<int32_t> ({maxNumMaterials,
...
// Transform utility function for OptiX instances
inline void getWorldTransform (MatrixRowMajor34f& t, const Eigen::Affine3f& xform, const Eigen::Vector3f& scale)
{
...
// Helper overload that takes SpaceTime directly
inline void getWorldTransform (MatrixRowMajor34f& t, const sabi::SpaceTime& st)
{
    getWorldTransform(t, st.worldTransform, st.scale);
}

// Convenience overload using float array for compatibility with OptiX setTransform
inline void getWorldTransform(float transform[12], const Eigen::Affine3f& xform, const Eigen::Vector3f& scale)
{
    MatrixRowMajor34f t;
    getWorldTransform(t, xform, scale);
    
    // Copy to float array
    for (int i = 0; i < 12; ++i) {
        transform[i] = t.data()[i];
    }
}

// Convenience overload using float array and SpaceTime
inline void getWorldTransform(float transform[12], const sabi::SpaceTime& st)
{
    getWorldTransform(transform, st.worldTransform, st.scale);
}
