...
using ShockerMaterialHandlerPtr = std::shared_ptr<class ShockerMaterialHandler>;
...
{
 public:
    // Factory method to create a new ShockerMaterialHandler instance
    static ShockerMaterialHandlerPtr create (RenderContextPtr ctx)
    {
        return std::make_shared<ShockerMaterialHandler> (ctx);
    }

    // Constructor initializes the handler with texture samplers and context
    ShockerMaterialHandler (RenderContextPtr ctx);
