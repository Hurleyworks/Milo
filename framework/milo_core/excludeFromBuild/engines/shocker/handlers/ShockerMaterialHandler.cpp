...
ShockerMaterialHandler::ShockerMaterialHandler (RenderContextPtr ctx) :
    ctx (ctx)
{
    LOG (DBUG) << _FN_;
...
void ShockerMaterialHandler::initialize()
{
    if (isInitialized_)
    {
        LOG(WARNING) << "ShockerMaterialHandler already initialized";
        return;
    }
    
    LOG(INFO) << "Initializing ShockerMaterialHandler";
    ...
    isInitialized_ = true;
    LOG(INFO) << "ShockerMaterialHandler initialized successfully";
}

// Safely releases all texture and material resources
void ShockerMaterialHandler::finalize()
{
    if (!isInitialized_)
        return;
        
    LOG(INFO) << "Finalizing ShockerMaterialHandler";
    ...
    try
    {
        ...
        LOG (INFO) << "ShockerMaterialHandler finalized";
    }
...
optixu::Material ShockerMaterialHandler::createOptixMaterial()
{
    optixu::Material mat = ctx->getOptiXContext().createMaterial();

    // NOTE: Hit groups will be set by ShockerEngine after pipelines are created
    // This differs from production code where PipelineHandler is accessible via Handlers
    
    return mat;
}

optixu::Material ShockerMaterialHandler::createDisneyMaterial (const CgMaterial& material,
                                                        const std::filesystem::path& materialFolder, CgModelPtr model)
{
    ...
    if (!isInitialized_)
    {
        LOG(WARNING) << "ShockerMaterialHandler not initialized - falling back to legacy mode";
...
void ShockerMaterialHandler::updateMaterial (
