...
// Based on RiPRRenderHandler but adapted for Shocker's specific needs

#include "../../../common/common_host.h"

class RenderContext;
using RenderContextPtr = std::shared_ptr<RenderContext>;
using RiPRRenderHandlerPtr = std::shared_ptr<class RiPRRenderHandler>;

class RiPRRenderHandler
{
public:
    // Factory method
    static RiPRRenderHandlerPtr create(RenderContextPtr ctx) 
    { 
        return std::make_shared<RiPRRenderHandler>(ctx); 
    }
    
    RiPRRenderHandler(RenderContextPtr ctx);
    ~RiPRRenderHandler();
    ...
    // Get pick info buffer for reading (similar to RiPR renderer)
    ...
    bool denoise(CUstream stream, bool isNewSequence, class RiPRDenoiserHandler* denoiserHandler, struct GPUTimerManager::GPUTimer* timer = nullptr);
