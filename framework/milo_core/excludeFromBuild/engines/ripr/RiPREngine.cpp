...
void RiPREngine::render (const mace::InputEvent& input, bool updateMotion, uint32_t frameNumber)
{
    if (!isInitialized_)
    {
        LOG (WARNING) << "RiPREngine not initialized";
        return;
    }
...
{
    LOG (INFO) << "RiPREngine::setupPipelines()";
    ...
    uint32_t maxPayloadDwords = std::max ({ripr_shared::PathTraceRayPayloadSignature::numDwords,
...
void RiPREngine::createPrograms()
{
    ...
    // Path Tracing Pipeline Programs
    {
        ...
        p.setNumMissRayTypes (ripr_shared::PathTracingRayType::NumTypes);
        p.setMissProgram (ripr_shared::PathTracingRayType::Closest,
                          pathTracePipeline_->programs.at (RT_MS_NAME_STR ("pathTraceBaseline")));
        p.setMissProgram (ripr_shared::PathTracingRayType::Visibility,
...
void RiPREngine::updateCameraSensor()
{
    ...
    // Use the linear beauty buffer from RiPRRenderHandler
