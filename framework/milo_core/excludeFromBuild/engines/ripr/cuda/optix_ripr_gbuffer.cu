...
CUDA_DEVICE_KERNEL void RT_RG_NAME(setupGBuffers)() {
    ...
    const Point2D prevRasterPos =
        ripr_plp.f->prevCamera.calcScreenPosition(hitPointParams.prevPositionInWorld)
...
CUDA_DEVICE_KERNEL void RT_CH_NAME(setupGBuffers)() {
    ...
    const InstanceData &inst = ripr_plp.s->instanceDataBufferArray[bufIdx][optixGetInstanceId()];
    const GeometryInstanceData &geomInst = ripr_plp.s->geometryInstanceDataBuffer[sbtr.geomInstSlot];
    const DisneyData &mat = ripr_plp.s->materialDataBuffer[geomInst.materialSlot];
    ...
    Point2D texCoord;
    {
        ...
        normalInWorld = normalize(transformNormalFromObjectToWorldSpace(shadingNormalInObj));
        texCoord0DirInWorld = transformVectorFromObjectToWorldSpace(texCoord0DirInObj);
        texCoord0DirInWorld = normalize(
            texCoord0DirInWorld - dot(normalInWorld, texCoord0DirInWorld) * normalInWorld);
        if (!normalInWorld.allFinite()) {
            normalInWorld = Normal3D(0, 0, 1);
    ...
    ReferenceFrame shadingFrame(normalInWorld, texCoord0DirInWorld);
