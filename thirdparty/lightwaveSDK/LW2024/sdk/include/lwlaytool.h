...
#define LWLAYOUTTOOL_VERSION 4

#define LWT_DIRTY_REQSHIFT  (1<<3)
#define LWT_DIRTY_REQALT    (1<<4)
 ...
 *
 * key     Gets called when the user presses a key.
 *       Return true if the tool swallowed the key or
 *       false to let the system handle it.
 *
 */
typedef struct st_LWLayoutToolFuncs {
    ...
    int         (*key)    (LWInstance, LWDualKey i);
} LWLayoutToolFuncs;
