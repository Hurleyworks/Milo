...
typedef struct st_LWViewportObjectAccess {
    int          (*showFrame   )( struct st_LWVPAHostAccess *access, uint32_t width, uint32_t height, float *rgb_buffer, uint32_t modulus_rgb, float *z_buffer, uint32_t modulus_z, uint32_t scale, uint32_t flags, float *minimum, float *maximum );
...
typedef struct st_LWViewportHandler {
    ...
    int          (*newSize     )( LWInstance, uint32_t, uint32_t );         /*!< Width and Height.                         */
    int          (*redraw      )( LWInstance );                                     /*!< Redraws the window.                       */
    int          (*setViewIndex)( LWInstance, int );                                /*!< View 0 to n.                              */
    int          (*setViewMode )( LWInstance, uint32_t );                       /*!< LVVIEWT.                                  */
    int          (*setAccess   )( LWInstance, LWViewportObjectAccess *, struct st_LWVPAHostAccess * );
    uint32_t (*flags       )( LWInstance );                                     /*!< LWViewFlags.                              */
    int          (*start       )( LWInstance, uint32_t );                       /*!< Start rendering.                          */
    ...
    LWCStringUTF8 (*menuName   )( LWInstance, uint32_t );                       /*!< Name  of menu entry.                      */
    int          (*menuGetPick )( LWInstance, uint32_t );                       /*!< Menu set entry selected.                  */
    int          (*menuSetPick )( LWInstance, uint32_t, int );                  /*!< Menu get entry selected.                  */
...
typedef LWViewportObjectCamera *LWViewportCamera(uint32_t view_index, uint32_t view_mode, int eye_to_render);
...
typedef struct st_LWViewportObjectMenu {
    uint32_t (*getFlags)( uint32_t );               /*!< View 0 to n.                             */
    int          (*setFlags)( uint32_t, uint32_t ); /*!< View 0 to n, LWViewportFlags.            */
