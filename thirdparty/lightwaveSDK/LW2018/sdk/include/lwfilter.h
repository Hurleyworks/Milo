...
#define LWIMAGEFILTER_VERSION   6
...
#define LWPIXELFILTER_VERSION   9
...
typedef struct st_LWFilterGlobal {
    const char **(*buffersAvailable)();
    const char **(*buffersInUse)();
    int          (*isBufferInUse)( const char *name );
} LWFilterGlobal;
...
typedef struct st_LWFilterAccess {
        ...
        LWFrame           frame;
        LWTime            start, end;
        int             (*layers  )( const char *type );
        float *         (*getLine )( const char *type, int layer, int y);
        void            (*setRGB  )(int x, int y, const LWFVector rgb);
        void            (*setAlpha)(int x, int y, float alpha);
        LWMonitor        *monitor;
} LWFilterAccess;

/* Note: Flags now returns an array of string entries.                      */
/* static const char *flags[] = { LWBUFFER_SPECIAL, LWBUFFER_LUMINOUS, 0 }; */

typedef struct st_LWImageFilterHandler {
        LWInstanceFuncs  *inst;
        LWItemFuncs      *item;
        LWError         (*process) (LWInstance, const LWFilterAccess *);
        const char **   (*flags)   (LWInstance);
...
typedef struct st_LWPixelAccess {
        double                        sx, sy;
        int                         (*layers )(const char *type);
        void                        (*getVal )(const char *type, int num, float *);
        void                        (*setRGBA)(const float[4]);
        void                        (*setVal )(const char *type, int num, float *);
