...
typedef struct st_LWDisplay3DHandler {
    ...
    int            (*setBuffer)( LWInstance, int renderEye, const char *group, const char *name, LWPixmapID  buffer, unsigned int scale );
    int            (*getBuffer)( LWInstance, int renderEye, const char *group, const char *name, LWPixmapID *buffer );
    const char   **(*buffers  )( LWInstance, int renderEye );
} LWDisplay3DHandler;
