...
typedef enum {           /* Tags to get/set panel attributes */
    ...
    CTL_VIEWERCOLORSPACE,       /* color space for viewing the color square. */
} cTag;
...
typedef struct st_ContextMenuFuncs {
    ...
    void*           (*cmenuGetSelectedData)(LWContextMenuID menu); // Get the data pointer associated with the index returned from the popup
} ContextMenuFuncs;
...
#define MOVE_CON_WO_LW(ctl, x, y) ((ival.intv.value = x - CON_LW(ctl), (*ctl->set)(ctl, CTL_X, &ival)),(ival.intv.value = y, (*ctl->set)(ctl, CTL_Y, &ival)) )
#define SIZE_CON_WO_LW(ctl, w, h) ((ival.intv.value = w + CON_LW(ctl), (*ctl->set)(ctl, CTL_W, &ival)),(ival.intv.value = h, (*ctl->set)(ctl, CTL_H, &ival)) )
