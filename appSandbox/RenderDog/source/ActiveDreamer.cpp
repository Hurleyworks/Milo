...
void ActiveDreamer::waitingForMessages()
{
    incoming.wait()
        ...
        .handle<QMS::onError> ([&] (QMS::onError const& msg)
                               {
                                   errMessage = msg.errorMessage;
                                   errSeverity = msg.level;
                                   state = &ActiveDreamer::onError; })

         .handle<QMS::loadImageList> ([&] (QMS::loadImageList const& msg)
                                       {
                                           filePaths = msg.imagePaths;
                                           state = &ActiveDreamer::loadImageList; })
...
// onError
void ActiveDreamer::onError()
{
    ErrMsg msg;
    msg.message = errMessage;
    msg.severity = errSeverity;
    impl->onError (msg);

    state = &ActiveDreamer::waitingForMessages;
}

void ActiveDreamer::loadImageList()
{
    try
    {
        impl->loadImagesAsync (filePaths);
    }
    catch (std::exception& e)
    {
        LOG (CRITICAL) << e.what();
